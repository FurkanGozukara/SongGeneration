...
class LeVoInference(torch.nn.Module):
    ...
    def forward(self, lyric: str, description: str = None, prompt_audio_path: os.PathLike = None, genre: str = None, auto_prompt_path: os.PathLike = None, gen_type: str = "mixed", params = dict(), 
        ...
        with torch.no_grad():
            with suppress_output():
                if melody_is_wav:
                    wav_seperate = model.generate_audio(tokens, pmt_wav, vocal_wav, bgm_wav, gen_type=gen_type, chunked=chunked, chunk_size=chunk_size, num_steps=num_steps, guidance_scale=guidance_scale, extend_stride=extend_stride_value, progress_callback=diffusion_chunk_progress)
                else:
                    wav_seperate = model.generate_audio(tokens, gen_type=gen_type, chunked=chunked, chunk_size=chunk_size, num_steps=num_steps, guidance_scale=guidance_scale, extend_stride=extend_stride_value, progress_callback=diffusion_chunk_progress)
